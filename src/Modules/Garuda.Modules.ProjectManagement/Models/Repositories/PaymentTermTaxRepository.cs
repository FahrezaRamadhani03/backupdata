// <copyright file="PaymentTermTaxRepository.cs" company="CV Garuda Infinity Kreasindo">
...
{
    public class PaymentTermTaxRepository : RepositoryBase<PaymentTermTax>, IPaymentTermTaxRepository
    {
        public async Task AddOrUpdate(PaymentTermTax model)
        {
        ...
        public async Task<PaymentTermTax> GetData(Guid id)
        {
            var data = await this.dbSet.FirstOrDefaultAsync(x => x.Id == id);
            return data;
        }

        public async Task<List<PaymentTermTax>> GetData()
        {
