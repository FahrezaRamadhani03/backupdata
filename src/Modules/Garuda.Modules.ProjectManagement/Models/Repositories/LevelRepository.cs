// <copyright file="LevelRepository.cs" company="CV Garuda Infinity Kreasindo">
...
namespace Garuda.Modules.ProjectManagement.Models.Repositories
{
    ...
    {
        public async Task AddOrUpdate(Level model)
        {
        ...
        public async Task<Level> GetData(Guid id)
        {
            var data = await this.dbSet.FirstOrDefaultAsync(x => x.Id == id);
            return data;
        }

        public async Task<List<Level>> GetData()
        {
            var data = await this.dbSet.ToListAsync();
            return data;
        }

        public async Task<Level> GetData(string name)
        {
