...
namespace Garuda.Modules.ProjectManagement.Models.Repositories
{
    ...
    {
        public async Task AddOrUpdate(Province model)
        {
        ...
        public async Task<Province> GetData(Guid id)
        {
            var data = await this.dbSet.Include(u => u.Cities).FirstOrDefaultAsync(x => x.Id == id);
            return data;
        }

        public async Task<Province> GetData(string name)
        {
            var data = await this.dbSet.Include(u => u.Cities).FirstOrDefaultAsync(x => x.Name.ToLower() == name.ToLower());
            return data;
        }

        public async Task<List<Province>> GetData()
        {
            var data = await this.dbSet.Include(u => u.Cities).ToListAsync();
            return data;
        }
